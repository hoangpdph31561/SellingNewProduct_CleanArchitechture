...
internal sealed class MongoEmployeeRepository : IEmployeeRepository
{
    ...
    // --- Read side -------------------------------------------------------------------------
    // No JOIN, so the per-employee order count is computed by loading the real-sale orders once
    // and grouping in memory. Text filters and sorting also happen in memory.
