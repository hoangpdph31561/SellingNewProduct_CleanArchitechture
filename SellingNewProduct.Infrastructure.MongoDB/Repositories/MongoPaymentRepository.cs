...
internal sealed class MongoPaymentRepository : IPaymentRepository
{
    ...
    // --- Read side -------------------------------------------------------------------------
    // The payment list is plain field comparisons, so it runs on the database. "Outstanding
    // orders" needs the amount paid per order (a JOIN in SQL); here it loads completed payments
    // and groups them in memory. Soft-deleted rows are excluded explicitly.
