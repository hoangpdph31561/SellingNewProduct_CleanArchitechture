...
internal sealed class MongoOrderRepository : IOrderRepository
{
    ...
    // --- Read side -------------------------------------------------------------------------
    // MongoDB has no relational JOIN across collections, so we load the documents we need and
    // "stitch" them together in memory with LINQ-to-objects. Same contract as SQL Server — only
        ...
        CancellationToken theCancellationToken = default)
    {
        ...
        // so those fall back to the default newest-first.
    ...
    /// or <c>null</c> when no name filter was supplied. An empty list means "name supplied but
    /// nobody matched" — the caller then returns no orders.
    /// </summary>
