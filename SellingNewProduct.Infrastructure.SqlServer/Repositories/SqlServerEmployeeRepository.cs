...
internal sealed class SqlServerEmployeeRepository : IEmployeeRepository
{
    ...
    // --- Read side -------------------------------------------------------------------------
    // The order count per employee is a correlated subquery (COUNT over Orders) that EF Core
    // pushes into the same SQL statement, so list/filter/sort/paging all run on the database.
