...
/// <summary>
/// SQL Server read side for customers. The plain list/search runs entirely on the database;
/// the "top customers" report is a JOIN Orders x Customers + GROUP BY pushed down. Soft-deleted
/// rows are excluded by the Global Query Filter.
/// </summary>
internal sealed class SqlServerCustomerReadRepository : ICustomerReadRepository
{
    private readonly AppDbContext myAppDbContext;

    public SqlServerCustomerReadRepository(AppDbContext theAppDbContext)
    {
