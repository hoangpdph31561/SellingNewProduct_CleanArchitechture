...
/// <summary>
/// SQL Server read side for payments. The list/search runs on the database; the "outstanding
/// orders" report is a JOIN Orders x Customers plus a correlated SUM of completed payments.
/// Soft-deleted rows are excluded by the Global Query Filter.
/// </summary>
internal sealed class SqlServerPaymentReadRepository : IPaymentReadRepository
{
    private readonly AppDbContext myAppDbContext;

    public SqlServerPaymentReadRepository(AppDbContext theAppDbContext)
    {
    ...
    public async Task<Payment?> GetByIdAsync(Guid theId, CancellationToken theCancellationToken = default)
    {
        ...
        return aRecord is null ? null : SellingNewProduct.Infrastructure.SqlServer.Mapping.PaymentMapper.ToDomain(aRecord);
    }
