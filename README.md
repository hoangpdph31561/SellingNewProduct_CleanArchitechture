...
└── SellingNewProduct.API                        # Controller, DTO, Validation → ref Domain + Infra
```
...
API ─────► Infrastructure.* ─────► Domain
 │                                   ▲
 └───────────────────────────────────┘   (API biết Domain để dùng interface/entity)
```

- **Domain** không reference bất kỳ project nào, không reference EF Core, không biết SQL/Mongo.
- **Infrastructure** triển khai (implement) các interface mà Domain định nghĩa.
